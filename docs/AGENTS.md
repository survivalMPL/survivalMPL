...
`# Fast check (~5 s) — run before starting work and after each change`` ``devtools``::`[`test`](https://devtools.r-lib.org/reference/test.html)`(``filter ``=`` ``"smoke"``)`` `` ``# Full suite — must pass before any commit`` ``devtools``::`[`test`](https://devtools.r-lib.org/reference/test.html)`(``)`
...
Use `dev/experiments/` for exploratory scripts, temporary investigation
folders, and prototype outputs that should not be part of the R package
build. The entire `dev/` tree is excluded via `.Rbuildignore`; move
anything production-ready into the appropriate package location before
